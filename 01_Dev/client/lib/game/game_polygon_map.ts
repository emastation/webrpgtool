...
module WrtGame {
  ...
  export class PolygonMap extends Map {
    ...
    public setupMesh(heightMapData_:any, mapWidth:number, mapHeight:number, textures:any):void {
      ...
      var that = this;
      this.textures.forEach(function(texture:any, texIndex){
      ...
      for (var y = 0; y < mapHeight + 2; y++) {
        for (var x = 0; x < mapWidth + 2; x++) {
          if(doesThisTypeExist(typeMapData[y][x], 'P')) {
            var platform = new MapPolygonPlatform(x, y, heightMapData_, getTypeParameter(typeMapData[y][x], 'P'));
